...
export default function Canvas({ tree, selectedId, onSelect }) {
  ...
  const calculateCanvasHeight = () => {
    ...
    tree.forEach(node => {
      ...
      else if (node.type === 'Input' || node.type === 'PasswordInput' || node.type === 'Label' || node.type === 'Button') defaultHeight = 60;
      const nodeHeight = typeof node.props?.height === 'number' ? node.props.height : defaultHeight;
  ...
  return (
    <div className="panel canvas-panel">
      ...
      <div className="panel-body">
        ...
        >
          {tree.length === 0 && (
            <div className="muted">Drag from palette to add. Drag components to move them around.</div>
          )}
          {tree.map(node => (
            <Node
              key={node.id}
    ...
    <div
      ...
      style={{
        ...
        transform: enableDrag
          ? (draggable.isDragging ? 'rotate(2deg) scale(1.06)' : 'none')
          : undefined,
        transition: enableDrag && draggable.isDragging ? 'none' : 'all 0.2s ease',
        width: typeof node.props?.width === 'number' ? node.props.width : (isContainer ? 'min(100%, 360px)' : undefined),
        minHeight: isContainer ? (typeof node.props?.height === 'number' ? node.props.height : 120) : undefined,
        height: !isContainer && typeof node.props?.height === 'number' ? node.props.height : undefined,
    ...
    >
      ...
      {!isContainer && (
        <div className="node-header">
          <div className="node-title">{node.type === 'Button' ? 'Button' : (node.label || node.type)} <span className="chip">{node.type}</span></div>
        </div>
      )}
      {renderPreview(node)}
        ...
        <div className="node-children">
          <SortableContext items={(node.children || []).map(c => c.id)} strategy={verticalListSortingStrategy}>
            {(node.children || []).map(child => (
              <SortableChild key={child.id} parentId={node.id} child={child} selectedId={selectedId} onSelect={onSelect} />
            ))}
          </SortableContext>
...
function SortableChild({ parentId, child, selectedId, onSelect }) {
  ...
  const combinedTransform = (translate || scale) ? `${translate}${scale}` : undefined;
  const style = {
  ...
  return (
    <div 
      ...
      {...listeners}
    >
      <Node node={child} selectedId={selectedId} onSelect={onSelect} insideParent={true} />
    </div>
  );
}
    ...
    case 'Form':
      return (
        <div style={{ textAlign: 'center', marginBottom: 16 }}>
          <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 18 }}>{node.props?.title || 'Login Form'}</div>
        </div>
      );
    case 'Card':
      return (
        <div style={{ textAlign: 'center', marginBottom: 8 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: '#64748b' }}>{node.props?.title || 'Card'}</div>
        </div>
      );
    case 'Logo':
      return (
        <div style={{ 
          ...
          borderRadius: '12px',
          background: 'linear-gradient(135deg, rgba(96, 165, 250, 0.05) 0%, rgba(96, 165, 250, 0.15) 100%)',
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
          justifyContent: 'center',
          minHeight: node.props?.size === 'large' ? '140px' : node.props?.size === 'small' ? '80px' : '110px',
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            {node.props?.displayType === 'image' && node.props?.imageUrl ? (
              <div style={{
                ...
                <div style={{
                  width: node.props?.size === 'large' ? '120px' : node.props?.size === 'small' ? '60px' : '80px',
                  height: node.props?.size === 'large' ? '120px' : node.props?.size === 'small' ? '60px' : '80px',
                  borderRadius: '8px',
                  overflow: 'hidden',
                  ...
                  <div style={{
                    ...
                    background: 'rgba(96, 165, 250, 0.1)',
                    color: '#60a5fa',
                    fontSize: node.props?.size === 'large' ? '24px' : node.props?.size === 'small' ? '16px' : '20px'
                  }}>
                    🖼️
                  </div>
                </div>
              ...
              <div style={{
                ...
                <div style={{
                  fontSize: node.props?.size === 'large' ? 36 : node.props?.size === 'small' ? 18 : 28,
                  fontWeight: '800',
                  color: '#1e40af',
