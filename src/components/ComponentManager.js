...
export default function ComponentManager({ tree, onSaveComponent, onLoadComponent, onDeleteComponent, savedComponents }) {
  ...
  const inferComponentName = (tree) => {
    if (!tree || (Array.isArray(tree) && tree.length === 0)) return 'GeneratedComponent';
  ...
  const handleGenerateCode = (component) => {
    ...
    const safeName = component.name && component.name.trim() ? component.name : inferComponentName(component.tree);
  ...
  const generateReactComponent = (node) => {
    ...
    switch (node.type) {
      ...
      case 'Input':
        ...
        componentCode += `  <input type="text" placeholder="${props.placeholder || 'Enter text...'}" className="input-field" ${inputStyle} />\n`;
        break;
      ...
      case 'PasswordInput':
        ...
        componentCode += `  <input type="password" placeholder="${props.placeholder || 'Enter password...'}" className="password-field" ${passwordStyle} />\n`;
        break;
      ...
      case 'Button':
        ...
        componentCode += `  <button className="btn btn-${props.variant || 'primary'}" ${buttonStyle}>${props.text || 'Button'}</button>\n`;
        break;
      ...
      case 'Label':
        ...
        componentCode += `  <label className="form-label" ${labelStyle}>${props.text || 'Label'}</label>\n`;
        break;
        
      case 'Logo':
        if (props.displayType === 'image' && props.imageUrl) {
          const logoStyle = `style={{ 
            maxWidth: '${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'}', 
            maxHeight: '${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'}',
            width: '${props.width || 'auto'}', 
            height: '${props.height || 'auto'}', 
            backgroundColor: '${props.backgroundColor || 'transparent'}',
            margin: '0 auto',
          ...
          componentCode += `  <img src="${props.imageUrl}" alt="${props.altText || 'Logo'}" className="logo logo-${props.size || 'medium'}" ${logoStyle} />\n`;
        } else {
          const logoStyle = `style={{ 
            maxWidth: '${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'}', 
            maxHeight: '${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'}',
          ...
          componentCode += `  <div className="logo logo-${props.size || 'medium'}" ${logoStyle}>${props.text || 'LOGO'}</div>\n`;
        }
        break;
    ...
    switch (node.type) {
      case 'Form':
        return `<form class="form-container">
            ...
            ${(node.children || []).map(child => generateVueNode(child)).join('\n            ')}
        </form>`;
      ...
      case 'Input':
        return `<input type="text" class="input-field" placeholder="${props.placeholder || 'Enter text...'}">`;
        
      case 'PasswordInput':
        return `<input type="password" class="password-field" placeholder="${props.placeholder || 'Enter password...'}">`;
      ...
      case 'Logo':
        if (props.displayType === 'image' && props.imageUrl) {
          return `<img src="${props.imageUrl}" alt="${props.altText || 'Logo'}" class="logo" style="max-width: ${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'}; max-height: ${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'}; object-fit: contain;">`;
        } else {
    ...
    switch (node.type) {
      case 'Form':
        ...
        return `<form class="form-container" ${formStyle}>
            <h2>${props.title || 'Form'}</h2>
            ${(node.children || []).map(child => generateHTMLNode(child)).join('\n            ')}
        </form>`;
      ...
      case 'Input':
        const inputStyle = `style="
          ...
          display: block;
        "`;
        return `<input type="text" class="input-field" placeholder="${props.placeholder || 'Enter text...'}" ${inputStyle}>`;
      ...
      case 'PasswordInput':
        const passwordStyle = `style="
          ...
          display: block;
        "`;
        return `<input type="password" class="password-field" placeholder="${props.placeholder || 'Enter password...'}" ${passwordStyle}>`;
      ...
      case 'Button':
        const buttonStyle = `style="
          ...
          display: block;
        "`;
        return `<button class="btn" ${buttonStyle}>${props.text || 'Button'}</button>`;
      ...
      case 'Label':
        const labelStyle = `style="
          ...
          display: block;
        "`;
        return `<label class="form-label" ${labelStyle}>${props.text || 'Label'}</label>`;
        
      case 'Logo':
        if (props.displayType === 'image' && props.imageUrl) {
          const logoStyle = `style="
            max-width: ${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'}; 
            max-height: ${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'};
            width: ${props.width || 'auto'}; 
            height: ${props.height || 'auto'}; 
            background-color: ${props.backgroundColor || 'transparent'};
            margin: 0 auto;
            display: block;
          "`;
          return `<img src="${props.imageUrl}" alt="${props.altText || 'Logo'}" class="logo" ${logoStyle}>`;
        } else {
          const logoStyle = `style="
            max-width: ${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'}; 
            max-height: ${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'};
  ...
  const generateJavaScriptComponent = (node) => {
    ...
    switch (node.type) {
      ...
      case 'Card':
        ...
        break;
      case 'Input':
        componentCode += `<input type="text" placeholder="${props.placeholder || 'Enter text...'}" class="input-field" />`;
        break;
      case 'PasswordInput':
        componentCode += `<input type="password" placeholder="${props.placeholder || 'Enter password...'}" class="password-field" />`;
        break;
      case 'Button':
        componentCode += `<button class="btn btn-${props.variant || 'primary'}">${props.text || 'Button'}</button>`;
        break;
      case 'Label':
        componentCode += `<label class="form-label">${props.text || 'Label'}</label>`;
        break;
      case 'Logo':
        if (props.displayType === 'image' && props.imageUrl) {
          componentCode += `<img src="${props.imageUrl}" alt="${props.altText || 'Logo'}" class="logo logo-${props.size || 'medium'}" style="max-width: ${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'}; max-height: ${props.size === 'large' ? '120px' : props.size === 'small' ? '60px' : '80px'};" />`;
        } else {
          componentCode += `<div class="logo logo-${props.size || 'medium'}">${props.text || 'LOGO'}</div>`;
        }
        break;
      default:
...
`;
  ...
  const downloadCode = async (codeFiles, componentName, language) => {
    try {
      ...
      alert(`Component "${componentName}" downloaded successfully as ${componentName}-${language}.zip`);
    } catch (error) {
          ...
          <div className="generate-form">
            ...
            <div className="form-actions">
              <button 
                ...
                onClick={() => handleGenerateCode({ tree, language: selectedLanguage })}
              >
                Generate & Download
              </button>
              <button className="btn btn-secondary" onClick={() => setShowGenerateForm(false)}>
                Cancel
              </button>
            </div>
          </div>
